import { IconPlayerPause, IconPlayerPauseFilled, IconPlayerPlay, IconPlayerPlayFilled, IconPlayerTrackNext, IconPlayerTrackNextFilled, IconPlayerTrackPrev, IconPlayerTrackPrevFilled } from "@tabler/icons-react"
import SvgHoverToggle from "./SvgHoverToggle"
import { useAudioPlayer } from "../GlobalAudioProvider";

const AudioControls = ({context}) => {
    const { isPlaying, toggleTrackPaused,playNextSong } = useAudioPlayer();
    return(
        <div className="audio-controls" context={(context.mobile) ? 'mobile' : 'desktop'}>
            {(!context.mobile) ? <SvgHoverToggle iconHovered={IconPlayerTrackPrevFilled} iconDefault={IconPlayerTrackPrev} /> : null}
